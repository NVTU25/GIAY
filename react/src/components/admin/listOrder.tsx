...
const ListOrder = () => {
    ...
    const getStatusName = (statusId:number) => {
        const statusName = status.find((item => item.id === statusId));
        return statusName ? statusName.value : 'Không xác định';
    }
    return (
        <div className='min-h-[300px] mb-[30px] px-6'>
            <h2 className='text-[22px] font-semibold pt-[30px] pb-[20px]'>🕰 Lịch sử đơn hàng</h2>
            {orders.length > 0 ? (
                <div className='grid gap-4'>
                    {orders.map((order:any) => (
                        <div key={order.id} className='p-4 h-[200px] bg-white shadow-md rounded-lg'>
                            ...
                            <div className='flex justify-between items-center'>
                                ...
                                <div className='mt-2 text-gray-700'>
                                    ...
                                    <div className='mt-2 relative w-full'>
                                        <Link to={``}> 
                                            <button className='absolute right-0 top-0 cursor-pointer w-[185px] bg-blue-400 text-white font-sans h-[40px] rounded text-[15px]'>Xem chi tiết đơn hàng</button>
                                        </Link>
